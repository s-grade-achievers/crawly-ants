...
class WorldMapApp:
    def __init__(self, root):
        ...
        clear_lines_button = ttk.Button(
            root, text="Clear Lines", command=self.clear_lines
        )
        clear_lines_button.pack()

        color_button = ttk.Button(
            root, text="Select Line Color", command=self.choose_line_color
        )
        color_button.pack()
    ...
    def load_map_image(self, file_path=None):
        ...
        try:
            ...
            self.map_image = self.map_image.resize((1200, 900), Image.LANCZOS)
        ...
        if True:
            ...
            for i in range(len(city_names) - 1):
                ...
                self.canvas.create_line(
                    x1, y1, x2, y2, fill=self.line_color, tags="line", width=6
                )

                self.city_info_label.config(
                    text=f"\t\tTotal Time: {total_distance:.2f} ms\n\t   Path taken : {', '.join(city_names)}"
                )
